...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        questionsList.add(QuestionModel("Что такое акция?","кредит","валюта","ценная бумага","скидка","ценная бумага"))
        questionsList.add(QuestionModel("Что такое биржа?","учреждение для заключения сделок","доска объявлений","банк","акции","учреждение для заключения сделок"))
        questionsList.add(QuestionModel("Как часто надо контролировать свои расходы?","несколько раз в год","ежедневно","раз в месяц","никогда","ежедневно"))
        questionsList.add(QuestionModel("Чем занимается ЦентроБанк?","эмиссия денег","выдача кредитов физлицам","купля-продажа валюты","выполнение платёжно-кассовых операций","эмиссия денег"))
        questionsList.add(QuestionModel("Как часто необходимо анализировать инвестиций ?","раз в год","раз в месяц","всегда","Никогда","всегда"))
    ...
    override fun onBackPressed() {
        if (backPressedTime + 2000 > System.currentTimeMillis()) {
            ...
            backToast = Toast.makeText(baseContext, "Нажмите дважды для выхода из приложения", Toast.LENGTH_SHORT)
            backToast?.show()
        }
